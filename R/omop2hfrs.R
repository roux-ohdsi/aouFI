...
#'
omop2hfrs <- function(con, eligible){
    ...
    condition_concept_ids <- tbl(con, "concept") %>%
        ...
        filter(concept_id %in% !!unique(categories_concepts$hfrs_concept_id)) %>%
        distinct()
    ...
    categories_concepts <- categories_concepts %>%
        mutate(hfrs_concept_id = as.integer(hfrs_concept_id)) %>%
        distinct(hfrs_concept_id, hfrs_category, hfrs_score)
