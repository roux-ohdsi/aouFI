...
#' @title OMOP to VAFI
#'
#' @description Generates table of VAFI-related condition occurances from an OMOP database.
...
omop2vafi <- function(con, eligible){
    ...
    categories_concepts <- getConcepts(index = "vafi")
    ...
    condition_concept_ids <- tbl(con, "concept") %>%
        ...
        filter(concept_id %in% !!unique(categories_concepts$vafi_concept_id)) %>%
        distinct()
